...
int main(void)
{
	...
	CurrentSense_SetParams(3.3f, 12, 41.0f, 0.002f);   // Vref, ADC bits, AMC gain, shunt
...
void speed_Sense(uint16_t angle, uint16_t mag, uint16_t err, bool ef) {
	if (!encoder_initialized) return;
	    ...
	    if (now - last_tick < 10) return;  // 100 Hz sensing
 ...
 void status_Display(void) {
	 static uint32_t last_print = 0;
	     ...
	     if (HAL_GetTick() - last_print >= 100)
	     {
	         ...
	         printf("RPM_cmd=%.1f  RPM_actual=%.1f  RPM_err=%.1f  IU=%.2f IV=%.2f IW=%.2f  Mode=%d\r\n",
	                rpm_cmd, actual_rpm, rpm_error,
	                g_iu, g_iv, g_iw,
	                ms.active_mode);
	     }
 }
 ...
 static void speed_Control_Update(void)
 {
	 static uint32_t last_speed_tick = 0;
	     ...
	     if ((now - last_speed_tick) < 10) return;   // 100 Hz speed loop
 ...
 static void MS_UpdateSlow(void)
 {
	 ...
	 const float down_th = 0.65f;       // tune
	    ...
	    if (v_util > up_th)
	    {
	        if (desired_mode < MS_MODE_3_36V) desired_mode++;
	    }
	    else if (v_util < down_th)
	    {
	        if (desired_mode > MS_MODE_1_12V) desired_mode--;
	    }
 ...
 void Control_Fast_25kHz(void)
 {
     ...
     /* 3) ensure torque-producing current reference comes from the PI loop
        The 100Hz speed_Control_Update() handles setting foc.iq_ref. We simply zero id_ref */
     foc.id_ref = 0.0f;
...
void SystemClock_Config(void) {
	RCC_OscInitTypeDef RCC_OscInitStruct = {0};
	  ...
	  RCC_OscInitStruct.HSI48State = RCC_HSI48_ON;   // required for USB if using HSI48
	  ...
	  /* PLLP/PLLQ not needed unless you route them to peripherals; keep safe defaults */
...
static void CRS_USB_ClockSync_Init(void)
{
  ...
  CRSInit.ReloadValue          = __HAL_RCC_CRS_RELOADVALUE_CALCULATE(48000000, 1000); // 1kHz SOF
